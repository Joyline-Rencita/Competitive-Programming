...
int getMoneySpent(int keyboards_count, int* keyboards, int drives_count, int* drives, int b) {
    int maxSpent = -1; // Initialize to -1 (impossible to spend any money)
    
    // Iterate through all keyboard prices
    for (int i = 0; i < keyboards_count; i++) {
        // Iterate through all USB drive prices
        for (int j = 0; j < drives_count; j++) {
            int totalCost = keyboards[i] + drives[j]; // Calculate total cost
            // If total cost is within budget and is greater than current maxSpent
            if (totalCost <= b && totalCost > maxSpent) {
                maxSpent = totalCost; // Update maxSpent
            }
        }
    }
    
    return maxSpent; // Return the maximum spent or -1 if no valid combination found
}
